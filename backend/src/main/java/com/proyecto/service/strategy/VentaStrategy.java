...
public class VentaStrategy implements DocumentStrategy {
    ...
    private List<ImpactoDocumentoDto> parseExcel(Workbook workbook) {
        ...
        // If no header found, start from row 0, else start from header + 1
        ...
        java.util.List<TempVentaRow> tempRows = new java.util.ArrayList<>();
        ...
        for (Row row : sheet) {
            ...
            tempRows.add(new TempVentaRow(code, qty));
        }
        ...
        for (TempVentaRow tempRow : tempRows) {
            ...
            impactos.add(ImpactoDocumentoDto.builder()
                    ...
                    .description(product != null ? product.getDescription() : "Product Not Found")
                    .stockActual(currentStock)
                    .stockNuevo(currentStock.subtract(diff))
    ...
    private static class TempVentaRow {
        String code;
        double qty;
    }
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (lineIdx <= 20 && (codeIdx == -1 || qtyIdx == -1)) {
                    ...
                    if (tempCodeIdx != -1 || tempQtyIdx != -1) {
                        codeIdx = tempCodeIdx;
                        qtyIdx = tempQtyIdx;
                        // This was the header line, so skip it
                        continue;
                    }
                }

                if (codeIdx == -1 && lineIdx > 20) {
                    // Check if we passed the preamble without finding headers, use defaults
                    codeIdx = 0;
                    qtyIdx = 1;
                }

                // If we haven't decided on indices yet (still in preamble), skip
                if (codeIdx == -1)
                    continue;
    ...
    public void apply(Documento documento, List<ImpactoDocumentoDto> impactos) {
        ...
        for (ImpactoDocumentoDto imp : impactos) {
            ...
            product.setStock(product.getStock().subtract(imp.getDiferenciaStock().abs()));
